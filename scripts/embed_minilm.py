...
OUTPUT_DIR = "embeddings/minilm"
MODEL_NAME = "nreimers/MiniLM-L6-H384-uncased"

def embed_with_minilm():
    ...
    for item in tqdm(functions, desc="Embedding"):
        ...
        metadata.append({"id": item["id"], "model": "minilm", "model_name": MODEL_NAME})

    # Save
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    save_embeddings_with_metadata(np.array(embeddings), metadata, OUTPUT_DIR)

    print(f"MiniLM embeddings saved to {OUTPUT_DIR}")
