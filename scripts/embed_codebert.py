...
OUTPUT_DIR = "embeddings/codebert"
MODEL_NAME = "microsoft/codebert-base"

def embed_with_codebert():
    ...
    for item in tqdm(functions, desc="Embedding"):
        ...
        metadata.append({"id": item["id"], "model": "codebert", "model_name": MODEL_NAME})

    # Save
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    save_embeddings_with_metadata(np.array(embeddings), metadata, OUTPUT_DIR)

    print(f"CodeBERT embeddings saved to {OUTPUT_DIR}")
