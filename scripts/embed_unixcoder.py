...
OUTPUT_DIR = "embeddings/unixcoder"
MODEL_NAME = "microsoft/unixcoder-base"

def embed_with_unixcoder():
    ...
    print("Embedding with UniXcoder...")
    for item in tqdm(functions, desc="Embedding"):
        ...
        metadata.append({"id": item["id"], "model": "unixcoder", "model_name": MODEL_NAME})

    # Save
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    save_embeddings_with_metadata(np.array(embeddings), metadata, OUTPUT_DIR)

    print(f"UniXcoder embeddings saved to {OUTPUT_DIR}")
    print(f"Shape: {np.array(embeddings).shape}")

if __name__ == "__main__":
    embed_with_unixcoder()
